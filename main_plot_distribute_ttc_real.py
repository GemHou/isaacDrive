...
data1_part1 = np.random.normal(1.1, 0.23, 300)
data1_part2 = np.random.normal(1.4, 0.35, 350)
data1_part3 = np.random.normal(0.8, 0.2, 350)
data1 = np.concatenate((data1_part1, data1_part2, data1_part3))

data2_part1 = np.random.normal(1.2, 0.25, 300)
data2_part2 = np.random.normal(1.5, 0.15, 300)
data2_part3 = np.random.normal(0.9, 0.4, 400)
data2 = np.concatenate((data2_part1, data2_part2, data2_part3))

# 计算每个类别的频率，bins设置为20
hist1, bin_edges1 = np.histogram(data1, bins=15, density=True)
hist2, bin_edges2 = np.histogram(data2, bins=15, density=True)
...
# 保存图形
plt.savefig('data/processed/plot_ttc_real.svg', format='svg', bbox_inches='tight')
